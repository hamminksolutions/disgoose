...
users
  id              uuid primary key
  email           text unique
  username        text unique   -- case-insensitive uniqueness enforced via a generated `username_lower` column + unique index on it (not a plain `lower(username)` index — that isn't queryable with a simple `.eq()`, and `ilike` would misfire on underscores already present in real usernames)
...
- `GET /api/albums/search?q=...` → proxies to MusicBrainz, caches results. Rate-limited per IP at **20 requests/minute** via a Vercel WAF dashboard rule (fixed window, no application code) — both to respect MusicBrainz's "reasonable use" etiquette and because this endpoint is reachable by anyone, not just logged-in users. See `docs/research/rate-limiting-vercel.md` for why WAF over Upstash/Postgres here.
- `POST /api/ratings` → creates or updates (upserts) a rating for the logged-in user. Rate-limited per user at **20 requests/5 minutes** via a Supabase Postgres UPSERT counter (reuses existing infra, globally consistent by construction, unlike Vercel WAF's per-region counters) — enough headroom for a fast bulk-entry session, tight enough to blunt a scripted spam loop.
- `/profile/:username` — **not a JSON API endpoint**; a Next.js Server Component page (`app/profile/[username]/page.tsx`) that calls `findUserByUsername` + `getProfileGrid` directly, the same direct-fetch pattern the owner's own profile page (`app/page.tsx`) already used before this endpoint was ever written down. Looks up `:username` case-insensitively (via the `username_lower` generated column, see section 4). No reserved-username blocklist — the route is namespaced under `/profile/:username`, not the root, so a username can't collide with an app route. A username that doesn't match any user calls `notFound()` for a genuine 404, distinct from a matched user with zero ratings (the existing empty-state view). **Public, no auth required.** Shows the grid (`ORDER BY created_at DESC LIMIT 40`) — score + listen method for every rating regardless of viewer; review text (fetched per-rating via `GET /api/ratings/:id` when a cover is clicked) is included only if the viewer is the owner or an accepted Friend — see that endpoint below. Important: this shows only the 40 most recent ratings; older ratings are **not** deleted, just not shown in the grid. Put an index on `ratings(user_id, created_at)` so this query stays fast as users rate more than 40 albums.
...
- `PUT /api/ratings/:id` → changes the score, listen method, and/or review of an existing rating (separate from the upsert-on-re-rating logic in `POST /api/ratings`). Shares the same per-user rate-limit counter as `POST /api/ratings`.

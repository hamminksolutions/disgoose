...
users
  id              uuid primary key
  email           text unique
  username        text unique   -- also needs a case-insensitive unique index (unique (lower(username))): the plain unique constraint alone lets "NielsH" and "nielsh" register as two different accounts
...
- `GET /api/users/:username/profile` → looks up `:username` case-insensitively (against the `lower(username)` index above). No reserved-username blocklist — the route is namespaced under `/profile/:username`, not the root, so a username can't collide with an app route. A username that doesn't match any user returns a genuine 404 (`notFound()`), distinct from a matched user with zero ratings (the existing empty-state view). **Public, no auth required** (v1 adds a public read-only profile view). Fetches the user + their ratings for the grid: `ORDER BY created_at DESC LIMIT 40`. Returns score + listen method for every rating regardless of viewer; review text is included only if the viewer is the owner or an accepted Friend (see `friendships`, section 4) — otherwise omitted from the response, not just hidden client-side. Important: this shows only the 40 most recent ratings; older ratings are **not** deleted, just not shown in the grid. Put an index on `ratings(user_id, created_at)` so this query stays fast as users rate more than 40 albums.

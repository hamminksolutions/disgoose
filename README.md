...
- **Staging** — `https://oouwnedzosgwptcczgls.supabase.co`. Separate from production, for trying schema migrations against non-production data before they touch real users. Connect via the same `.env.example` keys, pointed at this project's URL/keys (never commit them — set as Vercel Preview env vars or in a local `.env.staging`, gitignored like `.env.local`).
